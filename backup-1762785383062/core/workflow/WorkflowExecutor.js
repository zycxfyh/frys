...
export class AsyncWorkflowExecutor extends EventEmitter {
  ...
  async shutdown() {
    ...
    logger.info(`AsyncWorkflowExecutor ${this.executionId} shutdown completed`);
  }
  ...
  async executeWorkflow(inputs) {
    // 如果没有工作流定义，使用convertInputToTasks处理输入
    if (!this.definition) {
      const tasks = this.convertInputToTasks(inputs);
      if (tasks.length === 0) {
        return {
          success: false,
      ...
      return {
        ...
        executionId: this.executionId,
        outputs: results.length === 1 ? results[0].outputs : results,
        stats: this.stats,
      };
    }
