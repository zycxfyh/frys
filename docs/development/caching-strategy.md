...
frys 实现了全面的多级缓存策略，结合内存缓存和可选的Redis缓存，提供高性能的数据访问和智能的缓存管理。
...
```javascript
// 创建读密集型策略
const readHeavyStrategy =
  cacheService.createAccessPatternStrategy('read_heavy');

// 创建写密集型策略
const writeHeavyStrategy =
  cacheService.createAccessPatternStrategy('write_heavy');
```
