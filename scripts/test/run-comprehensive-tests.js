...
class ComprehensiveTestRunner {
  ...
  async phaseCodeQuality() {
    return this.runPhase('代码质量检查', async () => {
      ...
      if (testSummary.failed > 0) {
        throw new Error(`单元测试失败: ${testSummary.failed}/${testSummary.total} 个测试失败`);
      }
      ...
      return new Promise((resolve, reject) => {
        ...
        testProcess.on('close', (code) => {
          if (code === 0) {
            ...
            reject(new Error(`工业级测试失败 (退出码: ${code}): ${errorOutput}`));
          }
        });
  ...
  async phasePerformanceTest() {
    return this.runPhase('性能测试', async () => {
      ...
      const monitorProcess = spawn('node', [
        ...
        '--duration', '30000',
        '--server', this.targetUrl.replace('http://', '').split(':')[1] || '3000'
      ], {
        cwd: path.join(__dirname, '..'),
        stdio: ['pipe', 'pipe', 'pipe']
      });
      ...
      // 运行负载测试
      this.log('运行负载测试...');
      const loadTestProcess = spawn('node', [
  ...
  parseTestResults(output) {
    ...
    lines.forEach(line => {
      if (line.includes('Tests  ')) {
        const match = line.match(/Tests\s+(\d+)\s+failed\s*\|\s*(\d+)\s+passed/);
        if (match) {
  ...
  generateComprehensiveReport() {
    ...
    this.results.summary.duration = endTime - new Date(this.results.timestamp).getTime();

    const reportPath = path.join(this.reportsDir, 'comprehensive-test-report.json');
    const htmlReportPath = path.join(this.reportsDir, 'comprehensive-test-report.html');
...
<body>
  ...
  generateRecommendations() {
    ...
    if (failedPhases.length > 0) {
      recommendations.push(`${failedPhases.length} 个测试阶段失败，需要检查相关配置和代码`);
    }
  ...
  async runAllTests() {
    ...
    try {
      ...
      console.log(`成功率: ${(this.results.summary.completedPhases / this.results.summary.totalPhases * 100).toFixed(1)}%`);
  ...
  for (let i = 0; i < args.length; i++) {
    switch (args[i]) {
      ...
      case '-h':
        ...
        console.log('');
        console.log('选项:');
        console.log('  -u, --target-url <url>    目标服务器URL (默认: http://localhost:3000)');
