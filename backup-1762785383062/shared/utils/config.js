...
const loadEnvConfig = () => {
  ...
  for (const envFile of envFiles) {
    try {
      // 修复路径：指向 config/env/ 目录
      const envPath = join(__dirname, '../../../config/env/', envFile);
...
export const config = {
  ...
  // 消息队列配置
  messaging: {
    cluster: safeString(getEnvVar('NATS_CLUSTER', 'frys-prod'), 'frys-prod'),
  ...
  ai: {
    ...
    conversation: {
      ...
      defaultModel: safeString(
        getEnvVar('CONVERSATION_DEFAULT_MODEL', 'openai'),
        'openai',
      ),
      persistMemory: safeBoolean(
        getEnvVar('CONVERSATION_PERSIST_MEMORY', 'true'),
        true,
      ),
    ...
    memory: {
      ...
      maxContextLength: safeParseInt(
        getEnvVar('MEMORY_MAX_CONTEXT_LENGTH', '4000'),
        4000,
      ),
    },
  },
