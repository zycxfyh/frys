...
import { $ } from "zx";
import {
	generateLightweightConfig,
	validateConfig,
} from "../config/lightweight.config.js";
...
function calculateSummary(results) {
	...
	results.summary.warnings = checks.filter(
		(c) => c.status === "warning",
	).length;
}
...
function printResults(results) {
	...
	console.log("=".repeat(50));
	console.log(
		`📈 汇总: ${results.summary.passed}/${results.summary.total} 通过, ${results.summary.failed} 失败, ${results.summary.warnings} 警告`,
	);
}
