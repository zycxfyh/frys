...
export class DeepSeekService {
  ...
  async testConnection() {
    try {
      ...
      throw errorHandler.createError('DEEPSEEK_CONNECTION_FAILED', error.message);
    }
  }
  ...
  async getModels() {
    try {
      ...
      throw errorHandler.createError('DEEPSEEK_MODELS_FETCH_FAILED', error.message);
    }
  }
  ...
  handleError(error) {
    ...
    if (message.includes('500') || message.includes('502') || message.includes('503')) {
      return errorHandler.createError('DEEPSEEK_SERVER_ERROR', 'DeepSeek服务暂时不可用');
    }
