...
class IndustrialSecurityAuditor {
  ...
  async scanDependencies() {
    ...
    return new Promise((resolve, reject) => {
      const audit = spawn('npm', ['audit', '--json', `--audit-level=${this.options.severityThreshold}`], {
        cwd: process.cwd(),
      ...
      audit.on('close', (code) => {
        ...
        this.log(`⚠️ 发现 ${this.results.summary.totalVulnerabilities} 个依赖漏洞`, 'warning');
      } else {
        this.log(`⚠️ 依赖扫描不可用 (退出码: ${code})，跳过此检查`, 'warning');
  ...
  async analyzeCodeSecurity() {
    ...
    this.log(`✅ 代码安全分析完成 - 扫描了 ${this.results.summary.scannedFiles} 个文件`, 'success');
  }
  ...
  checkUnsafePatterns(content, filePath) {
    ...
    const unsafePatterns = [
      { pattern: /eval\s*\(/g, type: 'eval_usage', severity: 'critical', message: '使用eval()函数，存在代码注入风险' },
      { pattern: /Function\s*\(\s*['"][^'"]*['"]\s*\)/g, type: 'function_constructor', severity: 'high', message: '使用Function构造函数，可能存在代码注入风险' },
      { pattern: /innerHTML\s*=\s*[^=]/g, type: 'innerhtml_assignment', severity: 'medium', message: '直接设置innerHTML，可能存在XSS风险' },
      { pattern: /document\.write\s*\(/g, type: 'document_write', severity: 'high', message: '使用document.write，可能存在XSS风险' },
      { pattern: /localStorage\s*\.\s*setItem\s*\(\s*['"][^'"]*['"]\s*,\s*['"](password|token|secret)/gi, type: 'localstorage_sensitive', severity: 'medium', message: '在localStorage中存储敏感信息' },
      { pattern: /sessionStorage\s*\.\s*setItem\s*\(\s*['"][^'"]*['"]\s*,\s*['"](password|token|secret)/gi, type: 'sessionstorage_sensitive', severity: 'medium', message: '在sessionStorage中存储敏感信息' }
    ];
    ...
    unsafePatterns.forEach(({ pattern, type, severity, message }) => {
      ...
      while ((match = pattern.exec(content)) !== null) {
        // 测试文件中的危险模式降低严重程度
        const adjustedSeverity = isTestFile ? (severity === 'critical' ? 'high' : severity === 'high' ? 'medium' : 'low') : severity;
        this.results.codeAnalysisIssues.push({
          type,
  ...
  checkSqlInjection(content, filePath) {
    ...
    if (!filename.includes('database') && !filename.includes('db') &&
        ...
        !filename.includes('orm') && !filename.includes('query')) {
      return;
    }

    const sqlPatterns = [
      { pattern: /execute\s*\(\s*['"`][^'"`]*(\+|\${).*['"`]\s*\)/gi, type: 'sql_injection_execute', severity: 'high', message: '动态SQL执行可能存在注入风险' },
      { pattern: /query\s*\(\s*['"`][^'"`]*(\+|\${).*['"`]\s*\)/gi, type: 'sql_injection_query', severity: 'high', message: '动态SQL查询可能存在注入风险' }
    ];
  ...
  checkXssVulnerabilities(content, filePath) {
    const xssPatterns = [
      { pattern: /location\.(hash|search|href)\s*[^=]/g, type: 'location_xss', severity: 'medium', message: '直接使用location对象，可能存在DOM XSS风险' },
      { pattern: /window\.location\s*[^=]/g, type: 'window_location_xss', severity: 'medium', message: '直接使用window.location，可能存在开放重定向风险' },
      { pattern: /(src|href|action)\s*=\s*['"][^'"]*\$\{/g, type: 'attribute_injection', severity: 'high', message: '在HTML属性中注入变量，可能存在属性注入攻击' }
    ];
  ...
  checkCommandInjection(content, filePath) {
    const commandPatterns = [
      { pattern: /spawn\s*\(\s*[^,]+,\s*\[[^\]]*\$\{/g, type: 'spawn_injection', severity: 'critical', message: 'spawn命令参数中包含变量，存在命令注入风险' },
      { pattern: /exec\s*\(\s*['"`][^'"`]*(\$\{|\+).*['"`]/g, type: 'exec_injection', severity: 'critical', message: 'exec命令中包含变量，存在命令注入风险' },
      { pattern: /child_process\.execSync\s*\(\s*['"`][^'"`]*(\$\{|\+).*['"`]/g, type: 'execsync_injection', severity: 'critical', message: 'execSync命令中包含变量，存在命令注入风险' }
    ];
  ...
  auditDockerCompose(content, filePath) {
    ...
    if (portMatches) {
      portMatches.forEach(match => {
        if (match.includes('6379') || match.includes('27017') || match.includes('3306')) {
          issues.push({
    ...
    // 检查环境变量安全
    if (content.includes('environment:') && content.includes('PASSWORD') && !content.includes('${')) {
      issues.push({
  ...
  auditNginxConfig(content, filePath) {
    ...
    // 检查是否禁用不安全的HTTP方法
    if (!content.includes('if ($request_method !~ ^(GET|HEAD|POST|PUT|DELETE|OPTIONS)$ )')) {
      issues.push({
  ...
  auditEnvFile(content, filePath) {
    ...
    lines.forEach((line, index) => {
      ...
      if (trimmed && !trimmed.startsWith('#')) {
        ...
        // 检查默认值
        if (value.includes('your-') || value.includes('change-me') || value.includes('default')) {
          issues.push({
  ...
  generateRecommendations() {
    ...
    if (this.results.codeAnalysisIssues.some(i => i.severity === 'critical')) {
      recommendations.push({
        priority: 'critical',
        category: 'code',
   ...
   */
  checkAuditResults() {
    const hasCriticalIssues = this.results.codeAnalysisIssues.some(i => i.severity === 'critical') ||
                             this.results.securityIssues.some(i => i.severity === 'critical') ||
                             this.results.configurationIssues.some(i => i.severity === 'critical');

    const hasHighVulnerabilities = this.results.summary.critical > 0 || this.results.summary.high > 0;

    const totalIssues = this.results.codeAnalysisIssues.length +
    ...
    if (hasHighVulnerabilities) {
      this.log(`⚠️ 安全审计完成：发现 ${totalIssues} 个安全问题（含高危），建议修复`, 'warning');
    } else if (totalIssues > 0) {
      this.log(`ℹ️ 安全审计完成：发现 ${totalIssues} 个低危安全问题，可选修复`, 'info');
    } else {
