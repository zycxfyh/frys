...
class DataProcessingNode extends WorkflowNode {
  ...
  validate() {
    return (
      this.data.operation &&
      ['filter', 'transform', 'aggregate'].includes(this.data.operation)
    );
  }
  ...
  aggregateData(data) {
    ...
    switch (operation) {
      ...
      case 'avg':
        return (
          data.reduce((sum, item) => sum + (Number(item) || 0), 0) / data.length
        );
      case 'max':
...
class LoopNode extends WorkflowNode {
  ...
  validate() {
    return (
      this.data.loopType &&
      ['forEach', 'map', 'filter'].includes(this.data.loopType)
    );
  }
...
export class VisualWorkflowDesigner extends EventEmitter {
      ...
      nodes: [
        {
          ...
          data: { triggerType: 'webhook', schedule: 'immediate' },
        },
        {
          type: 'ai',
          position: { x: 300, y: 100 },
          data: { model: 'gpt-4', prompt: '分析客户查询并提供解决方案' },
        },
        {
          type: 'condition',
          position: { x: 500, y: 100 },
          data: { condition: (data) => data.confidence > 0.8 },
        },
        {
      ...
      nodes: [
        {
          type: 'trigger',
          position: { x: 100, y: 100 },
          data: { triggerType: 'schedule', schedule: 'daily' },
        },
        {
          type: 'data-processing',
          position: { x: 300, y: 100 },
        ...
        },
        {
          ...
          data: { model: 'gpt-4', prompt: '基于数据生成分析报告' },
        },
      ],
      connections: [
        {
  ...
  addNode(type, config = {}) {
    if (this.nodes.size >= this.config.maxNodes) {
      throw frysError.system(
        `Maximum nodes limit reached: ${this.config.maxNodes}`,
      );
    }
  ...
  addConnection(
    ...
    if (this.connections.size >= this.config.maxConnections) {
      throw frysError.system(
        `Maximum connections limit reached: ${this.config.maxConnections}`,
      );
    }
    ...
    logger.debug(
      `Added connection ${connection.id} from ${sourceNodeId} to ${targetNodeId}`,
    );
  ...
  loadTemplate(templateId) {
    ...
    template.connections.forEach((connConfig) => {
      ...
      if (sourceId && targetId) {
        this.addConnection(
          sourceId,
          connConfig.sourceHandle,
          targetId,
          connConfig.targetHandle,
        );
      }
    });
  ...
  async executeNodes(inputs, options) {
    ...
    // 初始化触发器节点的输入
    const triggerNodes = executionOrder.filter(
