...
describe('VCP系统集成测试', () => {
  ...
  describe('记忆网络与工作流集成', () => {
    it('应该能够在工作流执行中利用记忆网络', async () => {
      ...
      // 连接记忆节点
      await memoryNetwork.addEdge(memoryNode1.id, memoryNode2.id, {
        type: 'related',
        strength: 0.8,
      });
    ...
    it('应该能够在执行后更新记忆网络', async () => {
      const workflow = {
        ...
        connections: [{ from: 'task-executor', to: 'memory-updater' }],
      };
  ...
  describe('插件协议系统集成', () => {
    it('应该能够加载和执行插件', async () => {
      ...
      const pluginWorkflow = {
        ...
        connections: [{ from: 'data-input', to: 'plugin-processor' }],
      };
  ...
  describe('实时通信系统集成', () => {
    it('应该支持工作流执行状态的实时更新', async () => {
      ...
      // 订阅工作流状态更新
      const unsubscribe = realtimeComm.subscribe(
        'workflow-updates',
        (update) => {
