...
function getTestFiles() {
  try {
    const output = execSync('find tests -name "*.test.js"', { encoding: 'utf8' });
...
`;
  ...
  if (content.startsWith('/**')) {
    // 替换现有的头部注释
    updatedContent = headerComment + content.replace(/^\/\*\*[\s\S]*?\*\//m, '').trim();
  } else {
    // 添加新的头部注释
  ...
  afterEach(() => {
    // 重置系统状态
      ...
      updatedContent = updatedContent.slice(0, braceIndex) +
        `\n  afterEach(() => {
    // 重置系统状态
