...
class PerformanceBenchmark {
	...
	async runMemoryBenchmark() {
		...
		this.results.benchmarks.memory = {
			...
			average: {
				rss:
					memoryResults.rss.reduce((a, b) => a + b, 0) /
					memoryResults.rss.length,
				heapUsed:
					memoryResults.heapUsed.reduce((a, b) => a + b, 0) /
					memoryResults.heapUsed.length,
				heapTotal:
					memoryResults.heapTotal.reduce((a, b) => a + b, 0) /
					memoryResults.heapTotal.length,
				external:
					memoryResults.external.reduce((a, b) => a + b, 0) /
					memoryResults.external.length,
			},
		};

		console.log(
			`  峰值内存使用: ${(this.results.benchmarks.memory.peak.heapUsed / 1024 / 1024).toFixed(2)}MB`,
		);
	}
	...
	logComparisonResults(comparison) {
		...
		Object.entries(comparison.comparisons).forEach(([category, metrics]) => {
			...
			Object.entries(metrics).forEach(([metric, result]) => {
				...
				console.log(
					`  ${statusEmoji} ${metric}: ${result.percentChange.toFixed(2)}% (${result.status})`,
				);
			});
		});
	}
