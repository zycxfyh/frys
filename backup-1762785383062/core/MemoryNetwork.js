...
export class MemoryNetwork extends EventEmitter {
   ...
   * @returns {Promise<MemoryNode>} 创建的节点
   */
  async createNode(config) {
    const { type, content, metadata = {} } = config;
    const nodeId = `mem_${Date.now()}_${Math.random().toString(36).substr(2, 9)}`;

    const memoryNode = new MemoryNode(nodeId, type, content, metadata);
   ...
   * @param {boolean} options.includeAssociations - 是否包含关联结果
   * @param {string} options.searchType - 搜索类型: 'semantic', 'keyword', 'hybrid'
   * @returns {Array} 搜索结果
   */
  async search(options = {}) {
    ...
    try {
      ...
      switch (searchType) {
        case 'semantic':
          candidates = await this._semanticSearch(query, threshold);
          break;
        case 'keyword':
          candidates = this._keywordSearch(query, threshold);
          break;
        case 'hybrid':
        default:
          candidates = await this._hybridSearch(query, threshold);
          break;
      }
      ...
      if (timeWindow) {
        ...
        candidates = candidates.filter(candidate => {
          const node = this.memoryNodes.get(candidate.nodeId);
          return node && (now - node.metadata.createdAt) <= timeWindow;
        });
      }

      // 上下文增强搜索
      if (contextNodes.length > 0) {
        candidates = this._contextualSearch(candidates, contextNodes, query);
      }

      // 关联搜索
      let finalResults = candidates;
      if (includeAssociations) {
        finalResults = await this._associationSearch(candidates, limit, threshold);
      }

      // 排序和限制结果
      const results = finalResults
        .sort((a, b) => {
          // 多维度排序：相似度 + 重要性 + 关联度 + 时间衰减
          const aScore = this._calculateComprehensiveScore(a);
          const bScore = this._calculateComprehensiveScore(b);
      ...
      results.forEach(result => {
        const node = this.memoryNodes.get(result.nodeId);
        if (node) {
          node.metadata.accessCount++;
          node.metadata.lastAccessed = new Date();
          this._updateNodeImportance(node);
        }
      });
      ...
      logger.error('记忆搜索失败', error, { query, options });
      throw error;
    }
  }
   ...
   * 语义搜索 - 基于向量相似度
   */
  async _semanticSearch(query, threshold) {
    const queryVector = await this._generateQueryVector(query);
    const candidates = [];

    for (const [nodeId, node] of this.memoryNodes.entries()) {
      if (!node.vector) continue;

      const similarity = this._calculateVectorSimilarity(queryVector, node.vector);
   ...
   * 关键词搜索 - 传统文本匹配
   */
  _keywordSearch(query, threshold) {
    const queryTerms = this._tokenizeAndNormalize(query);
    const candidates = [];

    for (const [nodeId, node] of this.memoryNodes.entries()) {
      const contentTerms = this._tokenizeAndNormalize(node.content);
      const typeTerms = this._tokenizeAndNormalize(node.type);

      const similarity = this._calculateKeywordSimilarity(queryTerms, [...contentTerms, ...typeTerms]);
   ...
   * 混合搜索 - 结合语义和关键词搜索
   */
  async _hybridSearch(query, threshold) {
    const semanticResults = await this._semanticSearch(query, threshold * 0.7);
    const keywordResults = this._keywordSearch(query, threshold * 0.8);
    ...
    semanticResults.forEach(result => {
      result.similarity *= 1.2; // 语义搜索权重更高
   ...
   * 上下文增强搜索
   */
  _contextualSearch(candidates, contextNodes, query) {
    const contextNodeIds = new Set(contextNodes);
    ...
    return candidates.map(candidate => {
      ...
      contextNodeIds.forEach(contextId => {
        const associationStrength = this._getAssociationStrength(candidate.nodeId, contextId);
        contextScore += associationStrength;
      });

      // 基于上下文调整相似度
      candidate.similarity *= (1 + contextScore * 0.3);
   ...
   * 关联搜索 - 基于图关系的扩展搜索
   */
  async _associationSearch(candidates, limit, threshold) {
    const visited = new Set();
    const expandedResults = [...candidates];

    // 限制扩展深度，避免过度扩展
    const maxDepth = 2;
    const associationsPerNode = 3;

    for (const candidate of candidates) {
      if (visited.has(candidate.nodeId)) continue;
      visited.add(candidate.nodeId);

      const associations = await this._findAssociations(candidate.nodeId, associationsPerNode, threshold);

      for (const association of associations) {
        if (!visited.has(association.nodeId) && expandedResults.length < limit * 2) {
          association.similarity *= 0.7; // 关联结果权重降低
          association.type = 'association';
          expandedResults.push(association);
          visited.add(association.nodeId);
        }
      }
    }
   ...
   * 查找节点关联
   */
  async _findAssociations(nodeId, limit, threshold) {
    const associations = [];
    const node = this.memoryNodes.get(nodeId);

    if (!node) return associations;

    // 直接连接的节点
    for (const [targetId, connection] of node.connections.entries()) {
      const targetNode = this.memoryNodes.get(targetId);
      if (!targetNode) continue;

      const associationStrength = connection.strength;
      if (associationStrength >= threshold) {
        associations.push({
          nodeId: targetId,
          similarity: associationStrength,
   ...
   * 生成查询向量
   */
  async _generateQueryVector(query) {
    // 简化实现：基于关键词的向量生成
    // 在实际实现中，这里应该使用预训练的语言模型
    const terms = this._tokenizeAndNormalize(query);
    const vector = new Array(128).fill(0); // 128维向量

    terms.forEach((term, index) => {
      const hash = this._simpleHash(term);
   ...
   * 计算关键词相似度
   */
  _calculateKeywordSimilarity(queryTerms, contentTerms) {
   ...
   * 获取关联强度
   */
  _getAssociationStrength(nodeId1, nodeId2) {
    const node1 = this.memoryNodes.get(nodeId1);
    const node2 = this.memoryNodes.get(nodeId2);

    if (!node1 || !node2) return 0;

    const connection1to2 = node1.connections.get(nodeId2);
    const connection2to1 = node2.connections.get(nodeId1);

    if (connection1to2 && connection2to1) {
      return (connection1to2.strength + connection2to1.strength) / 2;
    } else if (connection1to2) {
      return connection1to2.strength * 0.8; // 单向连接权重降低
    } else if (connection2to1) {
      return connection2to1.strength * 0.8;
    }
  ...
  _calculateComprehensiveScore(result) {
    ...
    const recencyScore = Math.exp(-(Date.now() - node.metadata.createdAt) / (24 * 60 * 60 * 1000)); // 时间衰减
    const importanceScore = node.metadata.importance;
    const accessScore = Math.log(node.metadata.accessCount + 1) / Math.log(100); // 对数访问频率
   ...
   * 更新节点重要性
   */
  _updateNodeImportance(node) {
    const age = (Date.now() - node.metadata.createdAt) / (24 * 60 * 60 * 1000); // 天数
    const accessFrequency = node.metadata.accessCount / Math.max(age, 1);

    // 基于访问频率和内容质量更新重要性
    node.metadata.importance = Math.min(1.0,
      node.metadata.importance +
      accessFrequency * 0.01 +
      (node.content.length > 100 ? 0.05 : 0) // 内容丰富的节点权重更高
    );
  }
