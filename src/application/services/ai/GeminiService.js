...
export class GeminiService {
  ...
  async getModels() {
    try {
      ...
      throw errorHandler.createError('GEMINI_MODELS_FETCH_FAILED', error.message);
    }
  }
  ...
  estimateTokenUsage(inputMessages, outputContent) {
    ...
    const inputChars = inputMessages.reduce((total, msg) => total + msg.content.length, 0);
  ...
  getModelCapabilities(modelName) {
    const capabilities = {
      'gemini-1.5-pro': ['chat', 'completion', 'vision', 'audio', 'function-calling'],
  ...
  handleError(error) {
    ...
    if (message.includes('500') || message.includes('502') || message.includes('503')) {
      return errorHandler.createError('GEMINI_SERVER_ERROR', 'Gemini服务暂时不可用');
    }
