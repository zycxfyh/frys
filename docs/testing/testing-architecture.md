...
export default defineConfig({
  test: {
    coverage: {
      ...
      exclude: ['node_modules/', 'dist/', 'test/', 'docs/', '**/*.config.js'],
      thresholds: {
        global: {
          statements: 80,
          branches: 75,
...
describe('Workflow Engine Integration', () => {
  ...
  it('should execute user registration workflow', async () => {
    ...
    expect(workflow.tasks.every((task) => task.status === 'completed')).toBe(
      true,
    );
  });
...
test.describe('User Registration Flow', () => {
  ...
  test('should complete full user registration flow', async ({ page }) => {
    ...
    const verificationLink =
      await getVerificationLinkFromEmail('john@example.com');
...
test.describe('Workflow Management', () => {
  ...
  test('should create and execute workflow', async ({ page }) => {
    ...
    // 验证执行结果
    await expect(page.locator('[data-testid="task-0-status"]')).toContainText(
      '完成',
    );
    await expect(page.locator('[data-testid="task-1-status"]')).toContainText(
      '完成',
    );

    // 检查执行日志
    await page.click('[data-testid="view-logs-button"]');
    await expect(page.locator('[data-testid="execution-log"]')).toContainText(
      '工作流执行成功',
    );
  });
  ...
  test('should monitor workflow performance', async ({ page }) => {
    ...
    const successRate = await page
      .locator('[data-testid="success-rate"]')
      .textContent();
    ...
    await expect(page.locator('[data-testid="cpu-usage"]')).toContainText('%'); // 应显示CPU使用率
  });
});
```
...
describe('Input Validation Security Tests', () => {
  ...
  describe('Rate Limiting', () => {
    test('should enforce API rate limits', async () => {
      ...
      for (let i = 0; i < 150; i++) {
        // 假设限制是每分钟100次
        requests.push(request(app).get(endpoint).query({ q: 'test' }));
      }
