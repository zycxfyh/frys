...
import { AI_PROVIDERS, getProviderInfo, recommendProviders } from '../config/ai-providers.js';
...
export class AIProviderUI {
    ...
    return `
      <div class="provider-card ${isRecommended ? 'recommended' : ''} ${this.selectedProvider === provider.id ? 'selected' : ''}"
        ...
        <div class="provider-features">
          ${provider.features.slice(0, 3).map(feature => `<span class="feature-tag">${feature}</span>`).join('')}
        </div>
  ...
  bindEvents() {
    ...
    document.getElementById('retry-tests-btn')?.addEventListener('click', () => this.runTests());
  }
  ...
  renderDiscoveredModels() {
    ...
    if (container && grid && this.discoveredModels) {
      ...
      grid.innerHTML = this.discoveredModels.map(model => `
  ...
  async runTests() {
    ...
    try {
      ...
      this.showTestStatus('connection-test', 'error', `测试失败: ${error.message}`);
      document.getElementById('retry-tests-btn').style.display = 'inline-block';
    }
  }
  ...
  showTestStatus(testId, status, message) {
    ...
    const detailsEl = document.getElementById(`${testId.split('-')[0]}-details`);
  ...
  calculateAvgPrice(provider) {
    ...
    const avgPrice = models.reduce((sum, model) => sum + (model.input + model.output) / 2, 0) / models.length;
...
// 将样式添加到页面
if (typeof document !== 'undefined' && !document.querySelector('#ai-provider-styles')) {
