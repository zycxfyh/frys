...
const memoryPolicy = new MemoryScalingPolicy({
  scaleUpThreshold: 0.8, // 内存 > 80% 时扩容
  scaleDownThreshold: 0.3, // 内存 < 30% 时缩容
});
```
...
const requestPolicy = new RequestScalingPolicy({
  ...
  scaleDownThreshold: 0.2, // 请求率 < 20% 时缩容
});
```
...
import {
  CpuScalingPolicy,
  MemoryScalingPolicy,
} from './infrastructure/scaling/ScalingPolicy.js';
...
setInterval(() => {
  ...
  console.log(
    `负载均衡器状态: ${stats.loadBalancer.healthyInstances}/${stats.loadBalancer.totalInstances}`,
  );
}, 30000);
```
...
history.forEach((event) => {
  console.log(
    `${event.type}: ${event.fromInstances} -> ${event.toInstances} (${event.reason})`,
  );
});
```
