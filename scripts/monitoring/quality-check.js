...
class IndustrialQualityChecker {
	constructor(config = {}) {
		this.config = {
			maxConcurrency:
				config.maxConcurrency || Math.max(1, os.cpus().length - 1),
    ...
    */
		...
		return new Promise((resolve, reject) => {
			...
			child.on("close", (code) => {
				...
				if (code === 0) {
					...
					this.log(
						`${check.name} 失败 ✗ (退出码: ${code}, ${duration}ms)`,
						"error",
					);
			...
			child.on("error", (error) => {
				...
				this.log(
					`${check.name} 执行出错: ${error.message} (${duration}ms)`,
					"error",
				);
	...
	async runConcurrentChecks() {
		...
		const maxConcurrent = Math.min(
			this.config.maxConcurrency || os.cpus().length,
			this.checks.length,
		);
	...
	async runAllChecks() {
		...
		this.log(
			`📊 并发数: ${this.config.maxConcurrency}, 快速失败: ${this.config.failFast ? "启用" : "禁用"}, 缓存: ${this.config.cacheEnabled ? "启用" : "禁用"}`,
			"info",
		);
	...
	generateReport(results, totalDuration) {
		...
		const performanceStats = {
			...
			slowestCheck: results.reduce(
				(max, r) => (r.duration > max.duration ? r : max),
				{ duration: 0 },
			),
			fastestCheck: results.reduce(
		...
		return {
			...
			results: results.map((r) => ({
				...
				...(r.stderr &&
					r.stderr.length > 0 && { errorSnippet: r.stderr.substring(0, 200) }),
			})),
			systemInfo: {
	...
	printSummary(report) {
		...
		console.log(
			`⏱️  总耗时: ${(report.performanceStats.totalDuration / 1000).toFixed(2)}秒`,
		);
		...
		console.log("\n⚡ 性能统计:");
		console.log(
			`  平均耗时: ${(report.performanceStats.averageDuration).toFixed(0)}ms`,
		);
		if (report.performanceStats.slowestCheck.duration > 0) {
			console.log(
				`  最慢检查: ${report.performanceStats.slowestCheck.check.name} (${report.performanceStats.slowestCheck.duration}ms)`,
			);
		}
		if (report.performanceStats.fastestCheck.duration < Infinity) {
			console.log(
				`  最快检查: ${report.performanceStats.fastestCheck.check.name} (${report.performanceStats.fastestCheck.duration}ms)`,
			);
		}
...
const config = {
	...
	// 可以从环境变量读取默认配置
	maxConcurrency:
		argsConfig.maxConcurrency ||
		parseInt(process.env.QUALITY_MAX_CONCURRENCY) ||
		undefined,
	failFast:
		argsConfig.failFast !== undefined
			? argsConfig.failFast
			: process.env.QUALITY_FAIL_FAST !== "false",
	cacheEnabled:
		argsConfig.cacheEnabled !== undefined
			? argsConfig.cacheEnabled
			: process.env.QUALITY_CACHE_ENABLED !== "false",
