...
class PerformanceMonitor {
  constructor(options = {}) {
    ...
    this.outputFile = options.outputFile || path.join(__dirname, '../logs/performance-metrics.jsonl');
  ...
  getCPUUsage() {
    ...
    setImmediate(() => {
      ...
      return {
        ...
        percentage: ((endUsage.user + endUsage.system) / (elapsedTime * 1000000)) * 100 // 估算百分比
      };
    });
  ...
  generateReport() {
    ...
    console.log(`内存峰值: ${(report.memory.peak.heapUsed / 1024 / 1024).toFixed(2)}MB`);
  ...
  generateRecommendations() {
    ...
    // 内存使用建议
    if (memory.peak.heapUsed > 512 * 1024 * 1024) { // 512MB
    ...
    for (let i = 0; i < args.length; i++) {
      switch (args[i]) {
        ...
        default:
          ...
          console.log('  -o, --output <file>    输出文件 (默认: logs/performance-metrics.jsonl)');
          console.log('  -s, --server <port>    监控服务器进程 (默认端口: 3000)');
          process.exit(1);
      }
    }
