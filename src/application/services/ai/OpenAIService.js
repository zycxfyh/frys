...
export class OpenAIService {
  ...
  async getModels() {
    try {
      ...
      throw errorHandler.createError('OPENAI_MODELS_FETCH_FAILED', error.message);
    }
  }
  ...
  handleError(error) {
    ...
    if (message.includes('401')) {
      return errorHandler.createError('OPENAI_AUTH_FAILED', 'API密钥无效或过期');
    }

    if (message.includes('429')) {
      return errorHandler.createError('OPENAI_RATE_LIMIT', '请求频率过高，请稍后重试');
    }
    ...
    if (message.includes('500') || message.includes('502') || message.includes('503')) {
      return errorHandler.createError('OPENAI_SERVER_ERROR', 'OpenAI服务暂时不可用');
    }
