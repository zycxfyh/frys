...
class LoadGenerator {
	...
	createWorker(workerId) {
		return new Promise((resolve) => {
			const worker = spawn(
				"node",
				["-e", this.generateWorkerScript(workerId)],
				{
					cwd: __dirname,
	...
	generateDetailedReport() {
		const report = {
			...
			results: {
				...
				requestsPerSecond:
					this.stats.totalRequests / ((Date.now() - this.startTime) / 1000),
			},
	...
	static async stressTest(targetUrl, maxConcurrency = 50, duration = 30000) {
		...
		for (let concurrency = 5; concurrency <= maxConcurrency; concurrency += 5) {
			...
			results.push({
				...
				...generator.stats,
				successRate:
					generator.stats.totalRequests > 0
						? (generator.stats.successfulRequests /
								generator.stats.totalRequests) *
							100
						: 0,
				avgResponseTime:
					generator.stats.responseTimes.length > 0
						? generator.stats.responseTimes.reduce((a, b) => a + b, 0) /
							generator.stats.responseTimes.length
						: 0,
		...
		const reportPath = path.join(
			__dirname,
			"../reports/stress-test-report.json",
		);
		const reportsDir = path.dirname(reportPath);
		if (!fs.existsSync(reportsDir)) {
			fs.mkdirSync(reportsDir, { recursive: true });
...
if (import.meta.url === `file://${process.argv[1]}`) {
	const main = async () => {
		...
		if (args.length === 0) {
			...
			console.log("示例:");
			console.log(
				"  node load-generator.js http://localhost:3000 --duration 30000 --concurrency 20",
			);
