...
class CoverageTrendAnalyzer {
  ...
  async loadCoverageData() {
    ...
    // 加载历史趋势数据
    const trendsPath = path.join(rootDir, 'test-results', 'coverage-trends.json');
  ...
  async calculateTrends() {
    ...
    const trends = {
      ...
      status: {
        ...
        functions: this.getStatus(current.functions.pct, previous.functions?.pct),
        branches: this.getStatus(current.branches.pct, previous.branches?.pct),
        statements: this.getStatus(current.statements.pct, previous.statements?.pct),
      },
    };
  ...
  async generateTrendReport() {
    ...
    const reportPath = path.join(rootDir, 'test-results', 'coverage-trend-report.json');
  ...
  analyzeCoverageGaps() {
    ...
    Object.entries(current).forEach(([file, data]) => {
      ...
      if (this.previousCoverage) {
        ...
        if (prevData) {
          const declined = (data.lines.pct < prevData.lines.pct - 2) ||
  ...
  getCoverageIssues(data) {
    ...
    if (data.lines.pct < this.thresholds.lines) {
      issues.push(`行覆盖率 ${data.lines.pct}% 低于阈值 ${this.thresholds.lines}%`);
    }
    if (data.functions.pct < this.thresholds.functions) {
      issues.push(`函数覆盖率 ${data.functions.pct}% 低于阈值 ${this.thresholds.functions}%`);
    }
    if (data.branches.pct < this.thresholds.branches) {
      issues.push(`分支覆盖率 ${data.branches.pct}% 低于阈值 ${this.thresholds.branches}%`);
    }
  ...
  async generateRecommendations() {
    ...
    if (gaps.belowThreshold.length > 0) {
      recommendations.push({
        ...
        actions: gaps.belowThreshold.slice(0, 5).map(gap =>
    ...
    if (gaps.declining.length > 0) {
      recommendations.push({
        ...
        actions: gaps.declining.map(gap =>
  ...
  async generateImprovementSuggestions() {
    ...
    const suggestionsPath = path.join(rootDir, 'test-results', 'coverage-improvement-suggestions.json');
