...
export function getCostEffectiveProviders(limit = 5) {
  return Object.entries(AI_PROVIDERS)
    ...
    .sort((a, b) => {
      const aAvgCost =
        Object.values(a.pricing.models).reduce(
          (sum, model) => sum + (model.input + model.output) / 2,
          0,
        ) / Object.keys(a.pricing.models).length;
      const bAvgCost =
        Object.values(b.pricing.models).reduce(
          (sum, model) => sum + (model.input + model.output) / 2,
          0,
        ) / Object.keys(b.pricing.models).length;
...
export function recommendProviders(requirements = {}) {
  ...
  let candidates = Object.entries(AI_PROVIDERS).map(([id, provider]) => ({
    id,
    ...provider,
  }));
  ...
  if (features.length > 0) {
    candidates = candidates.filter((provider) =>
      features.every((feature) =>
        PROVIDER_FEATURES[feature]?.includes(provider.id),
      ),
    );
  }
  ...
  if (maxCost) {
    candidates = candidates.filter((provider) => {
      const avgCost =
        Object.values(provider.pricing.models).reduce(
          (sum, model) => sum + (model.input + model.output) / 2,
          0,
        ) / Object.keys(provider.pricing.models).length;
  ...
  // 按可靠性过滤
  candidates = candidates.filter(
    (provider) => provider.reliability >= minReliability,
  );
