...
export class CogneeMemoryService {
  constructor(options = {}) {
    ...
    this.baseURL = options.baseURL || config.ai.providers.cognee?.baseURL || 'https://api.cognee.ai';
    this.projectId = options.projectId || config.ai.providers.cognee?.projectId;

    // 测试模式检测
    this.isTestMode = this.apiKey === 'test-cognee-key' || this.apiKey?.startsWith('test-');
  ...
  async storeMemory(memoryData) {
    try {
      ...
      throw errorHandler.createError('COGNEE_MEMORY_STORE_FAILED', error.message);
    }
  }
  ...
  async retrieveMemory(query, options = {}) {
    try {
      ...
      throw errorHandler.createError('COGNEE_MEMORY_RETRIEVE_FAILED', error.message);
    }
  }
  ...
  async createKnowledgeGraph(graphData) {
    try {
      ...
      throw errorHandler.createError('COGNEE_GRAPH_CREATION_FAILED', error.message);
    }
  }
  ...
  async queryKnowledgeGraph(graphId, query, options = {}) {
    try {
      ...
      this.stats.errors++;
      logger.error('知识图谱查询失败', { error: error.message, graphId, query });
      ...
      throw errorHandler.createError('COGNEE_GRAPH_QUERY_FAILED', error.message);
    }
  }
  ...
  async storeConversationMemory(conversationData) {
    try {
      ...
      logger.error('对话记忆存储失败', { error: error.message, conversationData });
      throw errorHandler.createError('COGNEE_CONVERSATION_STORE_FAILED', error.message);
    }
  }
  ...
  async retrieveConversationMemory(query, conversationId, options = {}) {
    try {
      ...
      return {
        ...result,
        conversationId,
        relevantMessages: result.results.filter(memory =>
          memory.metadata.conversationId === conversationId
        )
      };

    } catch (error) {
      logger.error('对话记忆检索失败', { error: error.message, query, conversationId });
      throw errorHandler.createError('COGNEE_CONVERSATION_RETRIEVE_FAILED', error.message);
    }
  }
  ...
  async deleteMemory(memoryId) {
    try {
      ...
      throw errorHandler.createError('COGNEE_MEMORY_DELETE_FAILED', error.message);
    }
  }
