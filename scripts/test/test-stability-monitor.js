...
class TestStabilityMonitor {
  ...
  async runStabilityTests() {
    ...
    for (let i = 1; i <= this.runs; i++) {
      ...
      try {
        ...
        console.log(`✅ 第 ${i} 次运行成功: ${result.passed}/${result.total} 通过`);
  ...
  async analyzeResults() {
    ...
    if (analysis.totalRuns > 0) {
      ...
      if (durations.length > 0) {
        ...
        // 计算时长方差
        const variance = durations.reduce((acc, d) => {
          return acc + Math.pow(d - analysis.averageDuration, 2);
        }, 0) / durations.length;
    ...
    console.log(`⏱️  平均时长: ${(analysis.averageDuration / 1000).toFixed(1)}秒`);
  ...
  async generateStabilityReport() {
    ...
    const reportPath = path.join(rootDir, 'test-results', 'stability-report.json');
    fs.mkdirSync(path.dirname(reportPath), { recursive: true });
    fs.writeFileSync(reportPath, JSON.stringify(report, null, 2));

    // 生成人类可读的报告
    const summaryPath = path.join(rootDir, 'test-results', 'stability-summary.txt');
