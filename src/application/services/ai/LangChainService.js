...
class MockConversationChain {
  ...
  async call(inputs) {
    ...
    if (input.includes('你好') || input.includes('hello')) {
      ...
      response =
        '微服务是一种软件架构风格，将大型应用程序拆分为小型、独立的服务，每个服务运行在自己的进程中，可以独立部署和扩展。';
    }
  ...
  initializeModels(options) {
    try {
      ...
      this.models.claude = new MockChatAnthropic({
        anthropicApiKey:
          config.ai.providers.claude?.apiKey || 'test-claude-key',
      ...
      // DeepSeek - 总是初始化
      this.models['deepseek-chat'] = new MockChatOpenAI({
        openAIApiKey:
          config.ai.providers.deepseek?.apiKey || 'test-deepseek-key',
        modelName: options.deepseekModel || 'deepseek-chat',
        temperature: options.temperature || 0.7,
        maxTokens: options.maxTokens || 1000,
        streaming: options.streaming || false,
        configuration: {
          baseURL:
            config.ai.providers.deepseek?.baseURL || 'https://api.deepseek.com',
        },
      });

      // 为了兼容性，也添加deepseek别名
      this.models.deepseek = this.models['deepseek-chat'];
...
助手回复:`;
      ...
      const id =
        chainId ||
        `chain_${Date.now()}_${Math.random().toString(36).substr(2, 9)}`;
      ...
      throw errorHandler.createError(
        'LANGCHAIN_CHAIN_CREATION_FAILED',
        error.message,
      );
    }
  }
  ...
  async getConversationHistory(chainId) {
    try {
      ...
      throw errorHandler.createError(
        'LANGCHAIN_HISTORY_RETRIEVAL_FAILED',
        error.message,
      );
    }
  }
  ...
  async createCustomChain(chainConfig) {
    try {
      ...
      const id =
        chainId ||
        `custom_chain_${Date.now()}_${Math.random().toString(36).substr(2, 9)}`;
  ...
  async runCustomChain(chainId, inputs) {
    try {
      ...
      throw errorHandler.createError(
        'LANGCHAIN_CUSTOM_CHAIN_EXECUTION_FAILED',
        error.message,
      );
    }
  }
  ...
  async deleteChain(chainId) {
    try {
      ...
      throw errorHandler.createError(
        'LANGCHAIN_CHAIN_DELETION_FAILED',
        error.message,
      );
    }
  }
  ...
  listChains() {
    return Object.keys(this.chains).map((chainId) => {
      ...
      return {
        chainId,
        type:
          chain instanceof MockConversationChain ? 'conversation' : 'custom',
