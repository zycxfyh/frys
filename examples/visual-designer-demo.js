...
async function example1_basicWorkflow() {
  ...
  try {
    ...
    // 连接节点
    const conn1 = designer.addConnection(
      triggerNode.id,
      'output',
      aiNode.id,
      'input',
    );
    const conn2 = designer.addConnection(
      aiNode.id,
      'output',
      conditionNode.id,
      'input',
...
async function example2_templateWorkflow() {
  ...
  try {
    ...
    console.log(
      `✅ 模板加载完成: ${result.nodes.length} 个节点, ${result.connections.length} 个连接`,
    );

    // 自定义节点配置
    const aiNode = Array.from(designer.nodes.values()).find(
      (node) => node.type === 'ai',
    );
    if (aiNode) {
...
async function example3_dataProcessingPipeline() {
  ...
  try {
    ...
    console.log('📊 处理结果:', {
      ...
      aiAnalysis:
        result.results[aiNode.id]?.response?.substring(0, 100) + '...',
    });
  } catch (error) {
...
async function main() {
  ...
  try {
    ...
    console.log(
      '  2. 查看 docs/modules/future-blueprint-comprehensive.md 了解完整蓝图',
    );
