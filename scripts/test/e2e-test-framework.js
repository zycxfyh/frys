		...
		return this.runTest("AI对话完整流程", async (testResult) => {
			...
			try {
				...
				if (createResponse.success) {
					...
					testResult.steps[testResult.steps.length - 1].conversationId =
						conversationId;
				} else {
				...
				if (message1Response.success) {
					testResult.steps[testResult.steps.length - 1].status = "passed";
					testResult.steps[testResult.steps.length - 1].response =
						message1Response.data.message.content;
				} else {
				...
				if (message2Response.success) {
					testResult.steps[testResult.steps.length - 1].status = "passed";
					testResult.steps[testResult.steps.length - 1].response =
						message2Response.data.message.content;
				} else {
				...
				if (
					...
					historyResponse.data.messages.length >= 4
				) {
					testResult.steps[testResult.steps.length - 1].status = "passed";
					testResult.steps[testResult.steps.length - 1].messageCount =
						historyResponse.data.messages.length;
				} else {
	...
	async testLangChainIntegration() {
		return this.runTest("LangChain集成测试", async (testResult) => {
			...
			try {
				...
				const createResponse = await this.makeAPIRequest(
					"/api/ai/langchain/chains",
					"POST",
					{
						model: "openai",
						memoryType: "buffer",
						promptTemplate:
							"你是一个专业的{role}助手。{context}\n\n用户: {input}\n助手:",
					},
				);
				...
				if (converseResponse.success) {
					testResult.steps[testResult.steps.length - 1].status = "passed";
					testResult.steps[testResult.steps.length - 1].responseLength =
						converseResponse.data.response.length;
				} else {
	...
	async testCogneeMemorySystem() {
		return this.runTest("Cognee记忆系统测试", async (testResult) => {
			...
			if (storeResponse.success) {
				testResult.steps[testResult.steps.length - 1].status = "passed";
				testResult.steps[testResult.steps.length - 1].memoryId =
					storeResponse.data.memoryId;
			} else {
			...
			if (searchResponse.success && searchResponse.data.results.length > 0) {
				testResult.steps[testResult.steps.length - 1].status = "passed";
				testResult.steps[testResult.steps.length - 1].resultsCount =
			...
			const conversationResponse = await this.makeAPIRequest(
				"/api/ai/memory/store",
				"POST",
				{
		...
		return this.runTest("API健康检查测试", async (testResult) => {
			...
			for (const endpoint of endpoints) {
				...
				try {
					...
					if (
						response.success ||
						(endpoint.path === "/api/docs" && response.status === 200)
					) {
						testResult.steps[testResult.steps.length - 1].status = "passed";
					} else {
						testResult.steps[testResult.steps.length - 1].status = "failed";
						testResult.steps[testResult.steps.length - 1].error =
							`响应状态异常: ${response.status}`;
					}
				} catch (error) {
					testResult.steps[testResult.steps.length - 1].status = "failed";
			...
			// 检查是否有失败的步骤
			const failedSteps = testResult.steps.filter(
	...
	async testPerformanceBenchmarks() {
		return this.runTest("性能基准测试", async (testResult) => {
			const benchmarks = [
				...
				{
					name: "AI统计响应时间",
					endpoint: "/api/ai/providers/stats",
					threshold: 200,
				},
				{
					name: "对话创建响应时间",
					...
					method: "POST",
					body: { userId: "bench_user", model: "openai" },
					threshold: 500,
				},
			];
			...
			for (const benchmark of benchmarks) {
				...
				try {
					...
					if (response.success && responseTime <= benchmark.threshold) {
						testResult.steps[testResult.steps.length - 1].status = "passed";
						testResult.steps[testResult.steps.length - 1].responseTime =
							responseTime;
						...
						testResult.steps[testResult.steps.length - 1].responseTime =
							responseTime;
						testResult.steps[testResult.steps.length - 1].error =
							`响应时间过慢: ${responseTime}ms > ${benchmark.threshold}ms`;
					} else {
						testResult.steps[testResult.steps.length - 1].status = "failed";
	...
	for (let i = 0; i < args.length; i++) {
		switch (args[i]) {
			...
			case "-h":
				...
				console.log("选项:");
				console.log(
					"  -u, --base-url <url>    目标服务器URL (默认: http://localhost:3000)",
				);
				...
				console.log("示例:");
				console.log(
					"  node e2e-test-framework.js -u http://localhost:3000 -t 5000",
				);
				process.exit(0);
		}
	}
