...
async function multiProviderExample() {
  ...
  try {
    ...
    console.log('💰 成本优化路由结果:', {
      ...
      模型: 'gpt-3.5-turbo',
      回复:
        costEffectiveResponse.choices[0].message.content.substring(0, 100) +
        '...',
    });
    ...
    console.log('⚡ 速度优先路由结果:', {
      ...
      模型: 'gpt-4',
      回复:
        fastestResponse.choices[0].message.content.substring(0, 100) + '...',
    });
    ...
    Object.entries(allStats).forEach(([id, stats]) => {
      console.log(
        `  ${id}: 健康${stats.healthScore}/100, 请求${stats.totalRequests}次`,
      );
    });
  } catch (error) {
...
async function monitoringExample() {
  ...
  providerManager.manager.eventSystem.on('ai:call:completed', (data) => {
    console.log(
      `✅ AI调用完成: ${data.providerId} - ${data.duration}ms - 消耗${data.tokens.total_tokens}tokens`,
    );
  });
...
function loadProviderConfigsFromEnv() {
  ...
  if (process.env.ALIBABA_API_KEY) {
    configs.push({
      ...
      baseURL:
        process.env.ALIBABA_BASE_URL || 'https://dashscope.aliyuncs.com/api/v1',
    });
  }
...
async function saveProviderConfigs(providerManager) {
  ...
  const configs = Array.from(providerManager.providers.entries()).map(
    ([id, provider]) => ({
      id,
      name: provider.name,
      type: provider.type,
...
async function advancedExample() {
  ...
  providerManager.manager.eventSystem.on('ai:call:completed', (data) => {
    console.log(
      `✅ AI调用完成: ${data.duration}ms, ${data.tokens.total_tokens}tokens`,
    );
    // 可以在这里添加费用计算、使用统计等
  });
  ...
  console.log(
    '💬 上下文对话回复:',
    contextualResponse.choices[0].message.content.substring(0, 100) + '...',
  );
}
