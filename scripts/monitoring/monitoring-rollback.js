...
class MonitoringRollback {
	...
	generateReport() {
		...
		this.checks.forEach((check, index) => {
			...
			if (check.result) {
				console.log(
					`      结果: ${JSON.stringify(check.result, null, 2).replace(/\\n/g, "\\n           ")}`,
				);
			}
		...
		if (this.success) {
			...
			console.log(
				`📈 监控面板: ${process.env.MONITORING_URL || "https://monitoring.example.com"}`,
			);
			console.log(
				`📋 日志系统: ${process.env.LOGS_URL || "https://logs.example.com"}`,
			);
			console.log(
				`🚨 告警中心: ${process.env.ALERTS_URL || "https://alerts.example.com"}`,
			);
		} else {
