...
export class AIProviderManager {
  ...
  smartSelection(providers) {
    ...
    const scored = providers.map((provider) => {
      ...
      const speedScore = Math.max(
        0,
        1 - (provider.avgResponseTime || 1000) / 5000,
      );
      const costScore = Math.max(
        0,
        1 - this.calculateModelCost(provider.id, 'default') / 0.1,
      );
  ...
  async fallbackCall(request, excludeProviderId) {
    ...
    // 排除失败的供应商
    const fallbackProviders = this.findProvidersForModel(model).filter(
      (p) => p.id !== excludeProviderId,
    );
  ...
  calculateCost(providerId, model, usage) {
    ...
    const outputCost =
      (usage?.completion_tokens || 0) * (modelPricing.output || 0);
