...
class VCPSystemDemo {
    ...
    async demonstrateAgentWorkflowIntegration() {
        ...
        try {
            ...
            console.log(`✅ 创建Agent: ${dataAnalyzer.name} (ID: ${dataAnalyzer.id})`);
            ...
            console.log('🔄 执行工作流...');
            const result = await this.workflowExecutor.executeWorkflow(analysisWorkflow, {
                data: {
                    metrics: [
    ...
    async demonstrateRealtimeCommunication() {
        ...
        try {
            // 订阅工作流状态更新
            const unsubscribe = this.realtimeComm.subscribe('workflow-updates', (update) => {
    ...
    async demonstrateCompleteVCPSCollaboration() {
        ...
        try {
            ...
            const result = await this.workflowExecutor.executeWorkflow(collaborationWorkflow, {
                topic: '设计下一代智能工作流系统',
                userContext: {
