...
class RegressionMatrix {
	...
	async runSecurityTests() {
		await this.runTestSuite("安全测试", "npm run test:security", {
			timeout: 120000,
		});
	}
	...
	generateReport() {
		...
		const successRate = (
			(this.results.passed / (this.results.total - this.results.skipped)) *
			100
		).toFixed(1);
		...
		if (coreTestsPassed) {
			...
			console.log(
				"ℹ️  注意：某些测试在staging环境中被跳过或允许失败，这是正常现象。",
			);
		} else {
	...
	checkCoreTestsPassed() {
		...
		const coreSkipped = coreSuites.filter(
		...
		const actualPassRate =
			corePassed.length / (coreSuites.length - coreSkipped.length);
