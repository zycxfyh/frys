...
class SmartTestRunner {
  ...
  async validateEnvironment() {
    ...
    if (this.compareVersions(nodeVersion, requiredVersion) < 0) {
      throw new Error(`Node.js版本过低，需要${requiredVersion}或更高版本，当前版本${nodeVersion}`);
    }
  ...
  buildVitestCommand() {
    ...
    if (this.options.excludeTags.length > 0) {
      ...
      const excludePattern = this.options.excludeTags.map(tag => `(?=.*${tag})`).join('|');
    ...
    // 添加输出文件
    args.push('--reporter=json', '--outputFile=test-results/smart-runner-results.json');
