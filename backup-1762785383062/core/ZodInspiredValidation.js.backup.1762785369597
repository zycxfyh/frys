...
class StringValidator extends BaseValidator {
  validate(data, schema, context) {
    ...
    return (
      this.validateMinLength(data, schema, context) &&
      this.validateMaxLength(data, schema, context) &&
      this.validatePattern(data, schema, context) &&
      this.validateEnum(data, schema, context)
    );
  }

  validateMinLength(data, schema, context) {
  ...
  validateMaxLength(data, schema, context) {
  ...
  validatePattern(data, schema, context) {
  ...
  validateEnum(data, schema, context) {
    if (schema.enum && !schema.enum.includes(data)) {
      context.errors.push(
        `Value '${data}' is not in the allowed list: [${schema.enum.join(', ')}]`,
      );
      return false;
    }
    return true;
...
class NumberValidator extends BaseValidator {
  validate(data, schema, context) {
    ...
    const maxValue = schema.maximum !== undefined ? schema.maximum : schema.max;
    if (maxValue !== undefined && data > maxValue) {
      context.errors.push(`Number too large, maximum is ${maxValue}`);
      return false;
    }
